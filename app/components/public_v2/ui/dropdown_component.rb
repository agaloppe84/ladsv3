...
class PublicV2::Ui::DropdownComponent < ViewComponent::Base
  ...
  def initialize(label:, id: nil, align: :right, width: :md, classes: nil, trigger_classes: nil, panel_classes: nil, data: {}, debug: false)
    @label = label
    @id = id.presence || "pv2-dropdown-#{SecureRandom.hex(4)}"
    @align = normalize_option(align, ALIGNS, :right)
    @width = normalize_option(width, WIDTHS, :md)
    @classes = classes
