...
class PublicV2::Products::HeroComponent < ViewComponent::Base
  ...
  delegate :product, :quote_path, :catalog_path, to: :product_page

  def component_classes
    component_class_names(
      "pv2-product-hero pv2-product-hero-v2",
      "grid w-full min-w-0 grid-cols-1 items-stretch gap-4 min-[1121px]:grid-cols-[minmax(0,1fr)_minmax(380px,.78fr)]",
      debug_class
    )
  end

  def gallery_items
    product_page.hero_gallery_items
  end

  def gallery_modal?
    product_page.hero_gallery_modal?
  end
